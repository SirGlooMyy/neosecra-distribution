...
if [[ -n "$NEOSECRA_GHCR_USER" && -n "$NEOSECRA_GHCR_TOKEN" ]]; then
  ...
  if ! DOCKER_CONFIG="${DOCKER_CONFIG:-$HOME/.docker}" \
    docker manifest inspect "registry.neosecra.com/security-health-backend:${VERSION}" >/dev/null 2>&1; then
...
if [[ ! -f .env.v1 ]]; then
  ...
  printf '%s\n' \
    ...
    "WORKER_IMAGE=registry.neosecra.com/security-health-backend:${VERSION}" \
    "FRONTEND_IMAGE=registry.neosecra.com/security-health-frontend:${FRONTEND_IMAGE_VERSION}" \
